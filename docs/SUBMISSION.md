...
| 5 | 팀원 롤 기술서 | PDF (**개인 참가 시 생략**) | 🟡 템플릿 (`TEAM_ROLES.md`) — 팀이면 작성 |
...
- 추천 구성(45초): 타이틀 3초 → 게임 시작, 카드 해결 몽타주 30초(콤보·결재 도장
  강조) → 피날레 러시 8초 → 등급 도장 결과 화면 4초.
...
- 출품작은 순수 창작물이어야 하며 타인 저작물 무단 사용 시 실격. **원작 세계관
  문서(두뇌공화국)의 저작자가 팀 구성원이거나 서면 동의가 있어야 안전함.**
